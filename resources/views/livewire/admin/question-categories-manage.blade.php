                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150 {{ $category->trashed() ? 'bg-red-50' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex space-x-2">
                                    @if(!$category->trashed())
                                        <button
                                            wire:click="openModal(true, {{ $category->id }})"
                                        ...
                                        @if($category->subCategories->count() > 0)
                                            <button
                                                disabled
                                                class="text-gray-400 bg-gray-100 px-3 py-2 rounded-md flex items-center shadow-sm cursor-not-allowed"
                                                title="Tidak dapat dihapus karena memiliki subkategori">
                                                {{-- Font Awesome: fa-trash-can --}}
                                    ...
                                    @else
                                        ...
                                        @if($category->subCategories->count() > 0)
                                            <button
                                                disabled
                                                class="text-gray-400 bg-gray-100 px-3 py-2 rounded-md flex items-center shadow-sm cursor-not-allowed"
                                                title="Tidak dapat dihapus permanen karena memiliki subkategori">
                                                {{-- Font Awesome: fa-trash-can-xmark --}}
                                                <i class="fa-solid fa-trash-can-xmark w-4 h-4 mr-1"></i> Hapus Permanen
                                            </button>
                                        @else
                                            <button
                                                wire:click="confirmForceDelete({{ $category->id }})"
                        ...
                        <form wire:submit.prevent="{{ $isEdit ? 'update' : 'create' }}">
                            ...
                            <div class="flex justify-end gap-3 mt-8 pt-4 border-t border-gray-200">
                                ...
                                <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors duration-200 flex items-center">
                                    {{-- Font Awesome: fa-check / fa-floppy-disk --}}
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="delete-confirmation-title" role="dialog" aria-modal="true">
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-auto p-6">
                    ...
                    <p class="text-sm text-gray-500 text-center mb-6">Apakah kamu yakin ingin menghapus kategori ini? Data akan dipindahkan ke tempat sampah dan dapat dipulihkan kapan saja.</p>
    ...
    @if($confirmingForceDelete)
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="force-delete-confirmation-title" role="dialog" aria-modal="true">
            ...
            <div class="flex items-center justify-center min-h-screen p-4">
                <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full mx-auto p-6">
                    <div class="flex items-center justify-center w-12 h-12 mx-auto bg-red-100 rounded-full mb-4">
                        {{-- Font Awesome: fa-octagon-exclamation --}}
                    ...
                    <p class="text-sm text-gray-500 text-center mb-6">
                        <strong class="text-red-600">PERINGATAN:</strong> Tindakan ini akan menghapus kategori secara permanen.
