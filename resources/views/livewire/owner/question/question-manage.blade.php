<div>
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Manage Questions (Owner) - {{ $tryout->title }}</h1>
                            ...
                            @forelse($trashedQuestions as $index => $trashed)
                                <tr class="hover:bg-gray-50" wire:key="trashed-{{ $trashed->id }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        ...
                                        {{-- ✨ TOMBOL HAPUS PERMANEN (HANYA OWNER) ✨ --}}
                                        <button onclick="confirmForceDelete({{ $trashed->id }})" class="text-red-600 hover:text-red-900 bg-red-50 px-3 py-1 rounded transition-colors flex items-center inline-flex">
                                            <i class="fa-solid fa-trash-can w-3 h-3 mr-1"></i> Hapus Permanen
                                        </button>
                                    </td>
                                </tr>
                            @empty
    ...
    <script>
        ...
        // ✨ FUNGSI BARU: Konfirmasi hapus permanen (Khusus Owner) ✨
...
@push('scripts')
<script>
    // ✨ UBAH ROUTE TINYMCE KE OWNER ✨
    @if(Route::has('owner.tinymce.upload.image'))
        window.uploadImageUrl = "{{ route('owner.tinymce.upload.image') }}"; 
    @else
        window.uploadImageUrl = null;
        console.warn('Route "owner.tinymce.upload.image" not defined. Image upload in TinyMCE will not work.');
    @endif
